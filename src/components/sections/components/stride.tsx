...
export function StrideAnimatedCard({ project, className }: StrideAnimatedCardProps) {
  ...
  // Derive secondary color as a different shade of the same color (lighter variant)
  ...
  // Create a lighter shade by increasing brightness
  const secondaryRgb = {
  ...
  return (
    ...
    >
      ...
      <div className='h-[180px] w-full overflow-hidden'>
        <Visual1 mainColor={mainColor} secondaryColor={secondaryColor} gridColor={gridColor} />
      </div>

      {/* Body */}
      <div className='flex flex-col space-y-1.5 border-t-4 border-border p-3'>
        <H4 id='card-title' className='line-clamp-1'>
          {project._title}
        </H4>
        <M
          id='card-description'
