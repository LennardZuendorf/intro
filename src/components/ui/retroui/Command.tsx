  ...
  return (
    ...
    >
      ...
      <CommandPrimitive.Input
        className={cn(
          'flex h-11 w-full rounded bg-transparent py-3 text-sm outline-hidden text-foreground placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50 font-sans',
          className
        )}
        {...props}
      />
...
function CommandEmpty({ ...props }: React.ComponentProps<typeof CommandPrimitive.Empty>) {
  return (
    <CommandPrimitive.Empty
      ...
      className='py-6 text-center text-sm text-muted-foreground font-sans'
      {...props}
    />
  );
}
...
function CommandGroup({
  ...
  return (
    <CommandPrimitive.Group
      ...
      className={cn(
        'overflow-hidden p-1.5 text-foreground [&_[cmdk-group-heading]]:px-2 [&_[cmdk-group-heading]]:py-1.5 [&_[cmdk-group-heading]]:text-xs [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group-heading]]:text-muted-foreground font-sans',
        className
      )}
      {...props}
    />
...
const CommandShortcut = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => {
  return (
    <span
      ...
      className={cn('ms-auto text-xs tracking-widest text-muted-foreground font-sans', className)}
      {...props}
    />
  );
};
