...
type FloatingPanelPlacement = 'top' | 'bottom' | 'left' | 'right';
...
function useFloatingPanelLogic(initialPlacement: FloatingPanelPlacement = 'bottom') {
  const uniqueId = useId();
  const [isOpen, setIsOpen] = useState(false);
  const [note, setNote] = useState('');
  const [triggerRect, setTriggerRect] = useState<DOMRect | null>(null);
  const [title, setTitle] = useState('');
  // We'll store the placement in state (or you could just use a ref)
  const [placement] = useState<FloatingPanelPlacement>(initialPlacement);
...
interface FloatingPanelRootProps {
  ...
  className?: string;
  /**
   * Which side of the trigger to position the panel: "top", "bottom", "left", or "right"
   */
  placement?: FloatingPanelPlacement;
}
...
export function FloatingPanelRoot({
  ...
  const floatingPanelLogic = useFloatingPanelLogic(placement);
...
export function FloatingPanelContent({ children, className }: FloatingPanelContentProps) {
  ...
  // Close the panel on Escape press
  useEffect(() => {
  ...
  // Compute style based on placement
  ...
  if (triggerRect) {
    switch (placement) {
      ...
      case 'bottom':
        panelStyle = {
          ...panelStyle,
          left: triggerRect.left,
          top: triggerRect.bottom + 8, // offset
          transformOrigin: 'top left'
        };
        break;
      case 'left':
    ...
    // If no trigger rect, just center as a fallback
    panelStyle = {
      ...panelStyle,
      left: '50%',
      top: '50%',
...
export function FloatingPanelLabel({ children, htmlFor, className }: FloatingPanelLabelProps) {
  ...
  return (
    <motion.label
      ...
      className={cn('mb-2 block text-sm font-medium text-zinc-900 dark:text-zinc-100', className)}
    >
      {children}
    </motion.label>
  );
